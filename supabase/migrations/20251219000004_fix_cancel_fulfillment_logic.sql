-- Fix: Update cancel_fulfillment_and_return_stock to correctly update line items
...
BEGIN
    ...
    LOOP
        ...
        -- We need to ADD back to quantity_allocated because we just moved the reservation back to the order.
        -- Note: quantity_fulfilled is NOT touched here because the fulfillment was never shipped.
        -- If it was 'picking' or 'packed', it was never 'fulfilled' in the line item sense (which usually means shipped).
        -- Wait, if we track 'qty_in_fulfillment' separately, we don't need to touch quantity_fulfilled.
        -- BUT, we DO need to update quantity_allocated because we just added a ledger entry for the Order.
        
        UPDATE sales_order_lines
        SET quantity_allocated = quantity_allocated + v_line.quantity
    ...
    -- We explicitly check if the order is now fully allocated to set it to 'reserved'

-- Enhance product_inventory_view to include all necessary product details
-- This allows the ProductDetailView to use this view as the Single Source of Truth
...
SELECT
    p.id AS product_id,
    p.id, -- Redundant but useful for some ORMs/Clients expecting 'id'
    p.sku,
    p.name,
    p.description,
    p.status,
    ...
    -- This represents "Global Demand" (Committed to orders)
    COALESCE((

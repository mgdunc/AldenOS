-- 1. RPC to create fulfillment and move allocations
...
-- 2. Helper to get fulfillment quantities per line
CREATE OR REPLACE FUNCTION "public"."get_line_fulfillment_qty"("p_order_id" "uuid") 
RETURNS TABLE (line_id uuid, qty_in_fulfillment bigint, qty_shipped bigint)
LANGUAGE "plpgsql"
AS $$
BEGIN
    RETURN QUERY
    SELECT 
        fl.sales_order_line_id,
        COALESCE(SUM(CASE WHEN f.status != 'shipped' THEN fl.quantity ELSE 0 END), 0) as qty_in_fulfillment,
        COALESCE(SUM(CASE WHEN f.status = 'shipped' THEN fl.quantity ELSE 0 END), 0) as qty_shipped
    FROM fulfillment_lines fl
    JOIN fulfillments f ON fl.fulfillment_id = f.id
    WHERE f.sales_order_id = p_order_id
    GROUP BY fl.sales_order_line_id;
END;
$$;
